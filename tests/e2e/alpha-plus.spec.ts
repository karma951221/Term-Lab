...
test.describe.serial("★ 실물 재현 — 상품모델링을 화면으로 수행하고 조립 결과를 원문과 대조한다", () => {
  test(
    "상품 생성 → 보통약관 → 세목 → 별표 → 탑재(기본계약 1 + 특약 4) → 그룹 → 미리보기 대조",
    ...
    async ({ page, ev }) => {
      ...
      await ev.action("실물재현#1.3", "보통약관 템플릿을 고른다", async () => {
      ...
      await ev.action("실물재현#1.4", "세목 선택지 2건을 만든다 (종 축)", async () => {
      ...
      await ev.action("실물재현#1.5", "유효 조합 2건을 등록한다", async () => {
      ...
      await ev.action("실물재현#1.6", "별표 목록 21건을 저장한다 (순서가 곧 번호 — ADR-0030)", async () => {
      ...
      await ev.action("실물재현#1.12", "특약 그룹을 만든다", async () => {
      ...
      await ev.action("실물재현#1.17", "조립 미리보기를 연다 — 완성본이어야 한다", async () => {
  ...
  test(
    ...
    async ({ page, ev }) => {
      ...
      await ev.action("실물재현#2.3", "관을 넣는다", async () => {
