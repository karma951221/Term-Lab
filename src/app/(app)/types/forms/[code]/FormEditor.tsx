...
function typeLabel(type: FieldType, enums: EnumDef[]) {
  return type.kind === "enum" || type.kind === "list<enum>" ? `${TYPE_LABEL[type.kind]} (${enums.find((item) => item.code === type.enumCode)?.label ?? type.enumCode})` : TYPE_LABEL[type.kind];
}

function FieldTypeEditor({ field, enums, update }: { field: FormEditField; enums: EnumDef[]; update: (field: FormEditField) => void }) {
  const isChoice = field.type.kind === "enum" || field.type.kind === "list<enum>";
  return <span className="ts-row-actions"><select value={field.type.kind} onChange={(event) => { const kind = event.target.value as FieldType["kind"]; update({ ...field, type: kind === "enum" || kind === "list<enum>" ? { kind, enumCode: "" } : { kind } as FieldType, defaultValue: undefined }); }}>{Object.entries(TYPE_LABEL).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{isChoice ? <select value={"enumCode" in field.type ? field.type.enumCode : ""} onChange={(event) => { const kind = field.type.kind; if (kind === "enum" || kind === "list<enum>") update({ ...field, type: { kind, enumCode: event.target.value }, defaultValue: undefined }); }}><option value="">선택지 고르기</option>{enums.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}</select> : null}</span>;
}
...
function FieldsEditor({ code, enums, setEnums, usage }: { code: string; enums: EnumDef[]; setEnums: React.Dispatch<React.SetStateAction<EnumDef[]>>; usage: Record<string, number> }) {
  ...
  return <section className="ts-section"><h3 className="ts-section-title">필드 <span className="ts-count">{fieldState.value.length}</span></h3><table className="ts-table"><thead><tr><th className="col-num">순서</th><th className="col-code">코드</th><th className="col-flex">표시명</th><th className="col-fixed-md">타입</th><th>기본값 <InfoTip text={DEFAULT_INFO} /></th><th className="col-num">사용 수</th>{fieldState.mode === "edit" ? <th className="col-act">조작</th> : null}</tr></thead><tbody>{fieldState.value.map((field, index) => { const saved = !field.code.startsWith("new:"); return <tr key={field.code}><td className="col-num">{index + 1}</td><td className="col-code"><code>{saved ? field.code : "새 필드"}</code></td><td>{fieldState.mode === "read" ? <span className="ts-field-locked">{field.label}</span> : <input value={field.label} onChange={(event) => update({ ...field, label: event.target.value })} />}</td><td>{fieldState.mode === "read" ? typeLabel(field.type, enums) : <FieldTypeEditor field={field} enums={enums} update={update} />}</td><td>{fieldState.mode === "read" ? <span className="ts-field-locked">{field.defaultValue === undefined ? "없음" : Array.isArray(field.defaultValue) ? field.defaultValue.join(" · ") : String(field.defaultValue)}</span> : <DefaultValueInput type={field.type} value={field.defaultValue} onChange={(value) => update({ ...field, defaultValue: value })} enums={enums} />}</td><td className="col-num">{saved ? usage[field.code] || "—" : "—"}</td>{fieldState.mode === "edit" ? <td className="col-act"><span className="ts-row-actions"><IconButton icon={<IconUp />} label={`${field.label} 위로`} disabled={index === 0} onClick={() => move(index, -1)} /><IconButton icon={<IconDown />} label={`${field.label} 아래로`} disabled={index === fieldState.value.length - 1} onClick={() => move(index, 1)} />{saved ? <ConfirmActionButton label={`필드 ${field.label}(${field.code}) 삭제`} action={removeFormFieldEditAction.bind(null, code, field.code)} /> : <IconButton icon={<IconClose />} label={`${field.label} 추가 취소`} onClick={() => fieldState.setValue(fieldState.value.filter((item) => item.code !== field.code))} />}</span></td> : null}</tr>; })}</tbody></table>{fieldState.mode === "edit" ? <NewFieldComposer enums={enums} setEnums={setEnums} add={(field) => fieldState.setValue([...fieldState.value, field])} /> : null}</section>;
}
...
export function FormEditor({ def, initialEnums, usageCount, fieldUsage, valueRows }: { def: StructDiscriminator; initialEnums: EnumDef[]; usageCount: number; fieldUsage: Record<string, number>; valueRows: number }) {
  ...
  return <EditShell initial={initial} title={def.label} code={def.code} headerMeta={<><span className="ts-badge">{LEVEL_LABEL[def.level]}</span><span className="ts-count">사용처 <b>{usageCount}</b></span></>} saveAction={saveFormEditAction.bind(null, def.code)} deleteAction={removeFormEditAction.bind(null, def.code)} deleteLabel={`${def.label} 삭제`} deleteTooltip={`폼 ${def.label}(${def.code}) 삭제 — 저장된 값 ${valueRows}건이 사라진다`} deleteSuccessHref="/types/forms">
